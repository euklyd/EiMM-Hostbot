...
class Interview(commands.Cog):
    ...
    async def iv(self, ctx: commands.Context):
        ...
        if _server_active(ctx):
            reply = (
                f"Interviews are currently set up for {ctx.guild}; use `{ctx.bot.default_command_prefix}"
                f"help Interview` for more info."
            )
        else:
            reply = (
                f"Interviews are not currently set up for {ctx.guild}; use `{ctx.bot.default_command_prefix}"
    ...
    async def _check_sheet(self, ctx: commands.Context, sheet_name: str):
        ...
        if sheet_name not in sheet_names:
            await ctx.send(
                f"Spreadsheet `{sheet_name}` cannot be found, make sure it's been shared with the bot "
                f"account (`{ctx.bot.conf.google_email}`) and try again."
            )
            await ctx.message.add_reaction(ctx.bot.redtick)
            return False
    ...
    async def iv_channel(self, ctx: commands.Context, channel_type: str, channel: discord.TextChannel):
        ...
        if channel_type.lower() == "answer":
            ...
            await ctx.send(
                f"The only channels to set up are the `answer` ({answer.mention}) and "
    ...
    async def _ask_many(self, ctx: commands.Context, question_strs: List[str]):
        ...
        desc = (
            "\n".join(question_strs)[:1900] + "..."
            if len("\n".join(question_strs)) > 1900
            else "\n".join(question_strs)[0:1900]
        )
        em = discord.Embed(
            title=f"**{interviewee}**'s interview",
            description=desc,
    ...
    async def _channel_answer(self, ctx: commands.Context, channel: discord.TextChannel, preview_flag=False):
        ...
        for i, row in enumerate(rows):
            if row["Answer"] is not None and row["Answer"] != "" and row["Posted?"] == "FALSE":
                ...
                filtered_cells.append({"range": f"H{i + 2}", "values": [[True]]})
    ...
    async def vote(self, ctx: commands.Context, mentions: commands.Greedy[discord.Member]):
        ...
        class VoteError:
            ...
            def is_error(self):
                return (
                    len(self.opt_outs) > 0
                    or len(self.too_recent) > 0
                    or len(self.bots) > 0
                    or len(self.too_many) > 0
                    or self.self is True
                )
            ...
            async def send_errors(self):
                ...
                if len(self.bots) > 0:
                    reply += (
                        "• As much as I would love to usher in the **Rᴏʙᴏᴛ Rᴇᴠᴏʟᴜᴛɪᴏɴ**, you cannot vote for "
                ...
                if len(self.too_many) > 0:
                    reply += (
                        "• " + ", ".join([f"`{v}`" for v in self.too_many]) + " exceeded your limit of "
                        "three (3) votes.\n"
                    )
                await ctx.send(reply)
        ...
        for mention in mentions:
            votes.append(
                schema.Vote(
                    server_id=ctx.guild.id, voter_id=ctx.author.id, candidate_id=mention.id, timestamp=datetime.utcnow()
                )
            )
