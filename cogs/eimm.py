...
class EiMM(commands.Cog):
    ...
    def load(self) -> Dict[str, Dict]:
        ...
        new_keywords = {row["Keyword"]: row for row in keywords.get_all_records()}
        ...
        new_passives = {row["Ability"]: row for row in passives.get_all_records()}
    ...
    def _mod_bias_host_selection(hosts: Dict[str, dict], priority: int = 2) -> List[Host]:
        ...
        prio_hosts = [host for host in hosts if host.prio > 0]
        ...
        normal_hosts = [host for host in hosts_copy if host.prio >= 0]
        low_hosts = [host for host in hosts_copy if host.prio < 0]
